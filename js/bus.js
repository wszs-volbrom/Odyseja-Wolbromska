import { clamp, rectsOverlap, canvasFont, formatTime } from "./utils.js";

export const BUS_ROUTE_SEGMENTS = [
  {
    id: "wolbrom",
...
export class BusController {
  ...
  setTargetLane(lane, passengers) {
    ...
    if (this.speed > 260) passengers.changeSatisfaction(-1.4, "Szarpało przy zmianie pasa!");
  }
  ...
  render(ctx) {
    ...
    ctx.fillText("WOLBROM - KRAKÓW", this.width / 2, 48);
    ctx.restore();
  }
}
...
export class PassengerSystem {
  ...
  addPassengers(amount) {
    ...
    this.changeSatisfaction(5, "Nowi pasażerowie!");
  }
  ...
  updateDrivingComfort(dt, bus) {
    ...
    if (bus.targetLane < 2) {
      ...
      if (this.oppositeLaneTimer > 0.8) {
        this.changeSatisfaction(-10 * dt, "Jedziesz pod prąd!");
      }
    } else {
    ...
    if (bus.speed > 115 && bus.speed < 245 && bus.targetLane >= 2) {
      ...
      if (this.smoothTimer > 5) {
        this.changeSatisfaction(1.5, "Płynna jazda.");
        this.smoothTimer = 0;
      }
    } else {
      this.smoothTimer = 0;
  ...
  renderHUD(ctx, game, bus, routeLength, routeName, segmentName) {
    ...
    ctx.fillText(`Pasażerowie: ${this.passengerCount}/${this.passengerTarget}`, game.canvas.width * 0.34, 28);
  ...
  update(dt, bus, lanes, passengers, system) {
    ...
    if (inZone && bus.speed < 42) {
      ...
      if (this.stopHoldTime >= 1) {
        ...
        system.showText(`+${this.waitingPassengers} pasażerów`, this.x, lanes.laneCenter(this.lane) - 82, "#80ff9b");
    ...
    if (!this.collected && bus.x > this.x + this.stopZone + 190) {
      this.missed = true;
      passengers.changeSatisfaction(-13, "Pominięto przystanek!");
      system.showText("Pominięto przystanek!", this.x, lanes.laneCenter(this.lane) - 92, "#ef476f");
    }
  }
  ...
  render(ctx, bus, lanes) {
    ...
    if (!this.collected && !this.missed) {
      ...
      if (this.isBusNear(bus)) {
        ...
        ctx.lineWidth = 5;
        ctx.strokeText("Zatrzymaj się...", x, bayY - 12);
        ctx.fillStyle = "#ffe07a";
        ctx.fillText("Zatrzymaj się...", x, bayY - 12);
      }
    }
    ctx.restore();
  }
...
export class BusRideLevelSystem {
  constructor(level, canvas) {
    ...
    this.routeName = level.route || "WOLBROM -> KRAKÓW";
    this.texts = [];
    this.obstacles = [];
  ...
  update(dt, game) {
    ...
    if (this.bus.condition <= 0) {
      game.gameOver("Bus nie dojechał do Krakowa.");
      return;
    }
    if (this.passengers.satisfaction <= 0) {
      game.gameOver("Pasażerowie zbuntowali się w busie.");
      return;
    }
  ...
  pickSceneryLabel(segment, type, n) {
    if (type === "sign") {
      if (segment.id === "krakow") return "KRAKÓW";
      if (segment.id === "skala") return "SKAŁA";
