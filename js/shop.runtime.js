(function () {
  ...
  const ODYSEJA_SKINS = window.ODYSEJA_SKINS;
  if (!ODYSEJA_SKINS) {
    throw new Error("Skin shop config failed to load. Make sure js/skins.runtime.js is included before js/shop.runtime.js.");
  }

  const ODYSEJA_SAVE = window.ODYSEJA_SAVE;
  if (!ODYSEJA_SAVE) {
    throw new Error("Skin shop save system failed to load. Make sure js/save.runtime.js is included before js/shop.runtime.js.");
  }

  const { DEFAULT_SKIN_ID, INITIAL_SKIN_WALLET, PLAYER_SKINS } = ODYSEJA_SKINS;
  ...
  window.ODYSEJA_SHOP = Object.freeze({
    SkinShop,
  });
})();
