(function () {
  ...
  if (!ODYSEJA_ASSETS || !ODYSEJA_UTILS) {
    throw new Error("Collectibles dependencies failed to load. Make sure assets and utils are included before js/collectibles.runtime.js.");
  }

  const { COLLECTIBLE_IMAGES } = ODYSEJA_ASSETS;
  const { canvasFont, rectsOverlap } = ODYSEJA_UTILS;
...
class CollectibleItem {
  ...
  window.ODYSEJA_COLLECTIBLES = Object.freeze({
    CollectibleItem,
  });
})();
