(function () {
  ...
  if (!ODYSEJA_CONFIG || !ODYSEJA_UTILS) {
    throw new Error("Systems dependencies failed to load. Make sure config and utils are included before js/systems.runtime.js.");
  }

  const { CONFIG } = ODYSEJA_CONFIG;
  const { clamp, lerp, rectsOverlap } = ODYSEJA_UTILS;
...
class CollisionSystem {
  ...
  window.ODYSEJA_SYSTEMS = Object.freeze({
    SpeedrunTimer,
    HungerSystem,
    CameraSystem,
    CollisionSystem,
