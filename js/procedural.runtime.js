(function () {
  ...
  if (!ODYSEJA_CONFIG) {
    throw new Error("Procedural config failed to load. Make sure js/config.runtime.js is included before js/procedural.runtime.js.");
  }

  const ODYSEJA_UTILS = window.ODYSEJA_UTILS;
  if (!ODYSEJA_UTILS) {
    throw new Error("Procedural utils failed to load. Make sure js/utils.runtime.js is included before js/procedural.runtime.js.");
  }

  const ODYSEJA_LEVELS = window.ODYSEJA_LEVELS;
  if (!ODYSEJA_LEVELS) {
    throw new Error("Procedural level helpers failed to load. Make sure js/levels.runtime.js is included before js/procedural.runtime.js.");
  }

  const ODYSEJA_COLLECTIBLES = window.ODYSEJA_COLLECTIBLES;
  if (!ODYSEJA_COLLECTIBLES) {
    throw new Error("Procedural collectibles failed to load. Make sure js/collectibles.runtime.js is included before js/procedural.runtime.js.");
  }

  const ODYSEJA_ENEMIES = window.ODYSEJA_ENEMIES;
  if (!ODYSEJA_ENEMIES) {
    throw new Error("Procedural enemies failed to load. Make sure js/enemies.runtime.js is included before js/procedural.runtime.js.");
  }

  const { CONFIG, COLLECTIBLE_TYPES, BUILDING_ASSETS } = ODYSEJA_CONFIG;
  const { clamp } = ODYSEJA_UTILS;
  const { getCampaignLevelForNumber } = ODYSEJA_LEVELS;
...
class ProceduralLevelGenerator {
  ...
  window.ODYSEJA_PROCEDURAL = Object.freeze({
